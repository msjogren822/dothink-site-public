...
// ===== Chat Functions =====
function getChatUserToken() {
    let token = localStorage.getItem('treehouse_chat_user_token');
    if (!token) {
        token = 'chat_' + Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
        localStorage.setItem('treehouse_chat_user_token', token);
    }
    return token;
}
...
async function sendChat() {
    ...
    const userToken = getChatUserToken();
    ...
    try {
        const res = await fetch('/.netlify/functions/treehouse-chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: msg, userToken })
        });
        const data = await res.json();
        
        if (data.status === 'busy') {
            document.getElementById('chat-status').textContent = data.message;
        } else {
            document.getElementById('chat-status').textContent = 'Scout is typing...';
            // Start polling for response
            pollChatResponse();
        }
    } catch (e) {
        document.getElementById('chat-status').textContent = 'Error sending message';
    }
}
...
async function pollChatResponse() {
    ...
    chatPollInterval = setInterval(async () => {
        try {
            const res = await fetch(`/.netlify/functions/treehouse-chat?userToken=${encodeURIComponent(userToken)}`);
            const data = await res.json();
            ...
            if (data.timedOut || (data.isBusy === false)) {
                document.getElementById('chat-status').textContent = '';
                clearInterval(chatPollInterval);
                chatPollInterval = null;
            }
